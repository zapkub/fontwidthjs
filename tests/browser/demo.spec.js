...
test.describe('FontWidth Demo', () => {
  ...
  test('should test diverse Thai text examples with width verification', async ({
    page,
  }) => {
    // Wait for font loading
    await page.waitForTimeout(3000);
    ...
    const examples = [
      {
        text: 'กรุงเทพมหานคร',
        maxWidth: 300,
        description: 'long-thai-place-name',
      },
      {
        text: 'เมื่อพรุ่งนี้',
        maxWidth: 250,
      ...
      },
      {
        text: 'Only ทางเดิน',
        maxWidth: 220,
        description: 'mixed-english-thai',
      },
      {
        text: 'ญี่ปุ่นและ pop',
        maxWidth: 280,
        description: 'japanese-and-pop',
      },
    ];
  ...
  test('should debug font rendering and width calculation in preview', async ({
    page,
  }) => {
    // Wait for font loading
    await page.waitForTimeout(3000);
    ...
    const [fontSize, fontFamily, maxWidth, containerWidth, textWidth] =
      await Promise.all([
        previewText.evaluate(el => getComputedStyle(el).fontSize),
    ...
    const hasProperFont =
      fontFamily.includes('NotoSansThaiLoaded') ||
      fontFamily.includes('NotoSerifThaiLoaded');
